...
import cv2 #библ  Opencv для машинного обучения, можно: размер, поворот,градаия, размытие,рисование и тд
...
one_layer_net = OneLayerNet(input_height * input_width, num_classes) #размер изображения, класс изображения(двойна , тройка и тд)
...
for epoch in range(num_epochs):
    ...
    loss = 0
    for m in range(train_generator.get_data_size()): #перебор векторов
...
#показываем насколько ожидаемый результат совпадает с реально распознанным числом
